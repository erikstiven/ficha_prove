...
function obtenerEmpresasConUafeActivas($oCon, $oIfx = null)
{
    ...
    if ($oCon) {
        ...
        try {
            ...
            error_log('UAFE ERROR obtenerEmpresasConUafeActivas PG: ' . $e->getMessage() . ' SQL=' . $sql);
        }
    }
    ...
    if ($oIfx && empty($empresas)) {
        ...
        try {
            ...
            error_log('UAFE ERROR obtenerEmpresasConUafeActivas IFX: ' . $e->getMessage() . ' SQL=' . $sql);
        }
    }
...
function registrarOmitirAlertasUafeHoy()
{
    if (session_status() !== PHP_SESSION_ACTIVE) {
        session_start();
    }
    ...
    $oReturn = new xajaxResponse();
    $_SESSION[UAFE_MODAL_OMITIR_SESION_KEY] = date('Y-m-d');
...
function obtenerResumenAlertasUafe($diasAviso = UAFE_DIAS_AVISO_VENCIMIENTO)
{
    ...
    ";
        ...
        try {
            if ($oCon->Query($sqlRecalcular) && $oCon->NumFilas() > 0) {
                $vencidos = intval($oCon->f('proveedores_por_recalcular'));
            }
        } catch (Exception $e) {
            error_log('UAFE ERROR obtenerResumenAlertasUafe recalcular: ' . $e->getMessage() . ' SQL=' . $sqlRecalcular);
        }

        try {
            if ($oCon->Query($sqlEvaluables) && $oCon->NumFilas() > 0) {
                $totalEvaluables = intval($oCon->f('total_evaluables'));
            }
        } catch (Exception $e) {
            error_log('UAFE ERROR obtenerResumenAlertasUafe evaluables: ' . $e->getMessage() . ' SQL=' . $sqlEvaluables);
        }
...
function obtenerFechaVencimientoUafe($idempresa, $id_clpv, $oCon)
{
    ...
    try {
        ...
        error_log('UAFE ERROR obtenerFechaVencimientoUafe: ' . $e->getMessage() . ' SQL=' . $sqlV . ' empresa=' . $idempresa . ' proveedor=' . $id_clpv);
    }
    ...
    if ($fecha === '') {
        $sqlFallback = "
            ...
            FROM saetprov
            WHERE tprov_cod_empr = $idempresa
              AND tprov_venc_uafe IS NOT NULL
        ";

        try {
            $fecha = consulta_string($sqlFallback, 'tprov_venc_uafe', $oCon, '');
        } catch (Exception $e) {
            error_log('UAFE ERROR obtenerFechaVencimientoUafe fallback: ' . $e->getMessage() . ' SQL=' . $sqlFallback . ' empresa=' . $idempresa);
        }
    }
...
function obtenerMapaVencidosUafeVirtual($empresas, $oCon)
{
    ...
    $sql = "
        ...
        LEFT JOIN (
            ...
            FROM saetprov
            WHERE tprov_venc_uafe IS NOT NULL
    ...
    try {
        ...
        error_log('UAFE ERROR obtenerMapaVencidosUafeVirtual: ' . $e->getMessage() . ' SQL=' . $sql);
    }
...
function recalcularEstadosUafeGlobal()
{
    ...
    $sqlEvaluados = "
        SELECT COUNT(DISTINCT p.clpv_cod_clpv) AS total_evaluados
        FROM saeclpv p
        JOIN comercial.adjuntos_clpv a
          ON a.id_clpv = p.clpv_cod_clpv
         AND a.id_empresa = p.clpv_cod_empr
    ...
    $sqlAfectados = "
        SELECT COUNT(DISTINCT p.clpv_cod_clpv) AS total_afectados
        FROM saeclpv p
        JOIN comercial.adjuntos_clpv a
          ON a.id_clpv = p.clpv_cod_clpv
         AND a.id_empresa = p.clpv_cod_empr
    ...
    try {
        if ($oCon->Query($sqlEvaluados) && $oCon->NumFilas() > 0) {
            $resumen['evaluados'] = intval($oCon->f('total_evaluados'));
        }
    } catch (Exception $e) {
        error_log('UAFE ERROR recalcularEstadosUafeGlobal evaluados: ' . $e->getMessage() . ' SQL=' . $sqlEvaluados);
    }

    try {
        if ($oCon->Query($sqlAfectados) && $oCon->NumFilas() > 0) {
            $resumen['total_afectados'] = intval($oCon->f('total_afectados'));
            $resumen['cambiados_pendiente'] = $resumen['total_afectados'];
        }
    } catch (Exception $e) {
        error_log('UAFE ERROR recalcularEstadosUafeGlobal afectados: ' . $e->getMessage() . ' SQL=' . $sqlAfectados);
    }

    try {
        $oCon->Query($sqlUpdate);
    } catch (Exception $e) {
        error_log('UAFE ERROR recalcularEstadosUafeGlobal update: ' . $e->getMessage() . ' SQL=' . $sqlUpdate);
    }

    $resumen['fecha_hora'] = date('Y-m-d H:i:s');
    $_SESSION['uafe_ultima_ejecucion'] = $resumen['fecha_hora'];
